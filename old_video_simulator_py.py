...
def main():
    ...
    UV_kernel_fir = [
        ...
        -2.278365406039326e-02 ,
        -1.559268733007751e-18 ,]
    UV_kernel = RC_filter_kernel(input = [1], kernel_taps = 32, cutoff = 0.4)
    ...
    output_image[:, :, 0] = np.clip(output_image[:, :, 0], 0, 255);
    output_image[:, :, 1] = np.clip(output_image[:, :, 1], 0, 255);
    output_image[:, :, 2] = np.clip(output_image[:, :, 2], 0, 255);
